...
Proceed to create a new network. Select "*Configure Adapter Manually*" without changing any of the preset options.  

![image](https://github.com/user-attachments/assets/a0ba5eb2-85d9-4458-954c-607af4c7fe3b)

Then navigate to the "*DHCP Server*" tab and select "*Enalbe Server*" without making any changes.
...
Next, navigate to both VM's settings menu and select **Host-only Network** as the first network adapter.
![image](https://github.com/user-attachments/assets/7c023382-993d-4777-89ed-01a0942df180)

Start both the VMs and log in as user "**ubuntu**". 
...
**Caution:** This command is necessary for our VMs to acquire private network IP addresses and will need to be repeated **after every reboot**. To avoid having to repeat that, you can quit by saving the VMs' state after finishing your work session.

![image](https://github.com/user-attachments/assets/784bfdaf-c18b-4cc8-9e29-71021de8385d)
...
Following this, we can now use our host's terminal emulator to connect to the VMs via `ssh`:
```bash
...
For the controller and worker respectively.
...
Start HDFS using the following commands:
 ...
 start-dfs.sh
 ```
Confirm that the process was successful by accessing the web interface on the master's public IP using a browser: `http://192.168.56.5:9870`. 2 available live nodes should be accounted for.
...
Start YARN using the following command:
 ...
 start-yarn.sh
 ```
Confirm that the process was successful by accessing the web interface on the master's public IP using a browser: `http://192.168.56.5:8088/cluster`. The cluster should report 2 available live nodes.
...
Confirm that the process has been successful by accessing the web interface at the public IP of the master: `http://192.168.56.5:18080`.
...
```
You should be able to monitor the progress of the work in both the YARN web application (`http://192.168.56.5:8088`) and the history server (`http://192.168.56.5:18080`) after it is completed.
